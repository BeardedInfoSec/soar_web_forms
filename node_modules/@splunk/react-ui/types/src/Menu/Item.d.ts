...
type ItemFocusHandler = (event: React.FocusEvent<HTMLButtonElement>, data: {
    itemKey?: number;
}) => void;
interface ItemPropsBase {
    /**
     * Active shows a temporarily selected state, similar to that of focus. This is used when filtering the
     * `Menu` items in Multiselect, Select, and ComboBox and when navigating with arrows.
     */
    active?: boolean;
    ...
    /**
     * If `disabled` is set to `true`, the item is grayed out and cannot be clicked.
     * If set to `"dimmed"`, the menu item does not respond to events but can still receive focus, and
    ...
    hasSubmenu?: boolean;
    /**
     * @deprecated Icon is deprecated and will be removed in the next major version. Use `startAdornment` and/or `endAdornment` instead.
     *
     * An icon to insert before the children.
     *
     * Ignored if the `startAdornment` prop is provided.
     */
    icon?: React.ReactNode;
    /**
     * @private. The internal key of the item
     */
    itemKey?: number;
    /**
     ...
     * in <Combobox>, <Select>, and <Multiselect.Compact>
     */
    preventFocus?: boolean;
    /**
     * The default role is inferred from the other props. `selectable` with a
     * `selectableAppearance` of 'checkmark' defaults to `menuitemradio`. `selectable` with a
    ...
    /**
     * If selectable is true, whitespace is left where the checkmark can be shown.
     */
    selectable?: boolean;
    /**
     * If selectable is true, whitespace is left where the checkmark can be shown.
     ...
     * - With `selectableAppearance='checkmark'`: if `selected` is `true`, a checkmark shows the item is selected.
     * - With `selectableAppearance='checkbox'`:
     *   - `selected=true` renders a fully checked checkbox.
     *   - `selected='some'` renders a partially checked (indeterminate) checkbox, corresponding to `aria-checked='mixed'`.
     *   - `selected=false` renders an unchecked checkbox.
     */
...
declare const defaultProps: Required<Pick<ItemPropsBase, 'active' | 'descriptionPosition' | 'hasSubmenu' | 'openInNewContext' | 'preventFocus' | 'selectable' | 'selectableAppearance' | 'selected' | 'truncate'>>;
type ItemButtonProps = ClassComponentProps<ItemPropsBase, typeof defaultProps, 'button'>;
type ItemLinkProps = ClassComponentProps<ItemPropsBase, typeof defaultProps, 'a', 'href'>;
type ItemProps = ItemButtonProps | ItemLinkProps;
declare class Item extends Component<ItemProps, {}> {
    private el;
    static propTypes: React.WeakValidationMap<ItemProps>;
    static defaultProps: Required<Pick<ItemPropsBase, "active" | "selected" | "openInNewContext" | "descriptionPosition" | "hasSubmenu" | "preventFocus" | "selectable" | "selectableAppearance" | "truncate">>;
    static as: string;
    context: React.ContextType<typeof MenuContext>;
    static contextType: React.Context<import("./MenuContext").MenuContextValue>;
    static validateProps(props: Readonly<ItemProps>): void;
