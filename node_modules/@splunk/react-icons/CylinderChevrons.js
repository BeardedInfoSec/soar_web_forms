/******/ (() => {
    // webpackBootstrap
    /******/ "use strict";
    /******/ // The require scope
    /******/    var e = {};
    /******/
    /************************************************************************/
    /******/ /* webpack/runtime/compat get default export */
    /******/    (() => {
        /******/ // getDefaultExport function for compatibility with non-harmony modules
        /******/ e.n = t => {
            /******/ var r = t && t.__esModule ? 
            /******/ () => t["default"]
            /******/ : () => t
            /******/;
            e.d(r, {
                a: r
            });
            /******/            return r;
            /******/        };
        /******/    })();
    /******/
    /******/ /* webpack/runtime/define property getters */
    /******/    (() => {
        /******/ // define getter functions for harmony exports
        /******/ e.d = (t, r) => {
            /******/ for (var n in r) {
                /******/ if (e.o(r, n) && !e.o(t, n)) {
                    /******/ Object.defineProperty(t, n, {
                        enumerable: true,
                        get: r[n]
                    });
                    /******/                }
                /******/            }
            /******/        };
        /******/    })();
    /******/
    /******/ /* webpack/runtime/hasOwnProperty shorthand */
    /******/    (() => {
        /******/ e.o = (e, t) => Object.prototype.hasOwnProperty.call(e, t)
        /******/;
    })();
    /******/
    /******/ /* webpack/runtime/make namespace object */
    /******/    (() => {
        /******/ // define __esModule on exports
        /******/ e.r = e => {
            /******/ if (typeof Symbol !== "undefined" && Symbol.toStringTag) {
                /******/ Object.defineProperty(e, Symbol.toStringTag, {
                    value: "Module"
                });
                /******/            }
            /******/            Object.defineProperty(e, "__esModule", {
                value: true
            });
            /******/        };
        /******/    })();
    /******/
    /************************************************************************/    var t = {};
    // ESM COMPAT FLAG
        e.r(t);
    // EXPORTS
        e.d(t, {
        default: () => /* binding */ s
    });
    // CONCATENATED MODULE: external "react"
    const r = require("react");
    var n =  e.n(r);
    // CONCATENATED MODULE: external "@splunk/react-icons/SVG"
    const C = require("@splunk/react-icons/SVG");
    var l =  e.n(C);
    // CONCATENATED MODULE: external "@splunk/react-icons/IconProvider"
    const a = require("@splunk/react-icons/IconProvider");
    // CONCATENATED MODULE: ./src/icons/CylinderChevrons.tsx
    function o(e, t) {
        if (e == null) return {};
        var r = u(e, t);
        var n, C;
        if (Object.getOwnPropertySymbols) {
            var l = Object.getOwnPropertySymbols(e);
            for (C = 0; C < l.length; C++) {
                n = l[C];
                if (t.indexOf(n) >= 0) continue;
                if (!Object.prototype.propertyIsEnumerable.call(e, n)) continue;
                r[n] = e[n];
            }
        }
        return r;
    }
    function u(e, t) {
        if (e == null) return {};
        var r = {};
        var n = Object.keys(e);
        var C, l;
        for (l = 0; l < n.length; l++) {
            C = n[l];
            if (t.indexOf(C) >= 0) continue;
            r[C] = e[C];
        }
        return r;
    }
    var i = new Map;
    i.set("outlined", (function() {
        
        return n().createElement(n().Fragment, null,  n().createElement("path", {
            d: "M14.2071 11.2929C13.8166 10.9024 13.1834 10.9024 12.7929 11.2929C12.4024 11.6834 12.4024 12.3166 12.7929 12.7071L14.5858 14.5L12.7929 16.2929C12.4024 16.6834 12.4024 17.3166 12.7929 17.7071C13.1834 18.0976 13.8166 18.0976 14.2071 17.7071L16.495 15.4192C17.0027 14.9116 17.0027 14.0884 16.495 13.5808L14.2071 11.2929Z"
        }),  n().createElement("path", {
            d: "M11.2071 11.2929C10.8166 10.9024 10.1834 10.9024 9.79289 11.2929L7.50502 13.5808C6.99734 14.0884 6.99734 14.9116 7.50503 15.4192L9.79289 17.7071C10.1834 18.0976 10.8166 18.0976 11.2071 17.7071C11.5976 17.3166 11.5976 16.6834 11.2071 16.2929L9.41421 14.5L11.2071 12.7071C11.5976 12.3166 11.5976 11.6834 11.2071 11.2929Z"
        }),  n().createElement("path", {
            fillRule: "evenodd",
            clipRule: "evenodd",
            d: "M12 2C9.98341 2 8.1144 2.29032 6.71392 2.79049C6.01803 3.03902 5.38405 3.35829 4.90494 3.76283C4.43048 4.16346 4 4.74631 4 5.5V17.5C4 19.0493 5.22998 20.1828 6.60304 20.8693C8.03591 21.5857 9.94542 22 12 22C14.0546 22 15.9641 21.5857 17.397 20.8693C18.77 20.1828 20 19.0493 20 17.5V5.5C20 4.74631 19.5695 4.16346 19.0951 3.76283C18.616 3.35829 17.982 3.03902 17.2861 2.79049C15.8856 2.29032 14.0166 2 12 2ZM6.19525 5.29094C6.07042 5.39634 6.02321 5.46681 6.00633 5.5C6.02321 5.53319 6.07042 5.60366 6.19525 5.70906C6.42458 5.9027 6.81573 6.12215 7.38659 6.32603C8.5196 6.73067 10.1506 7 12 7C13.8494 7 15.4804 6.73067 16.6134 6.32603C17.1843 6.12215 17.5754 5.9027 17.8048 5.70906C17.9296 5.60366 17.9768 5.53319 17.9937 5.5C17.9768 5.46681 17.9296 5.39634 17.8048 5.29094C17.5754 5.0973 17.1843 4.87785 16.6134 4.67397C15.4804 4.26933 13.8494 4 12 4C10.1506 4 8.5196 4.26933 7.38659 4.67397C6.81573 4.87785 6.42458 5.0973 6.19525 5.29094ZM18 7.91726C17.7726 8.02403 17.5333 8.12123 17.2861 8.20951C15.8856 8.70968 14.0166 9 12 9C9.98341 9 8.1144 8.70968 6.71392 8.20951C6.46674 8.12123 6.22738 8.02403 6 7.91726V17.5C6 17.8837 6.33702 18.5002 7.49747 19.0804C8.5981 19.6308 10.1886 20 12 20C13.8114 20 15.4019 19.6308 16.5025 19.0804C17.663 18.5002 18 17.8837 18 17.5V7.91726Z"
        }));
    }));
    i.set("filled", (function() {
        
        return n().createElement(n().Fragment, null,  n().createElement("path", {
            fillRule: "evenodd",
            clipRule: "evenodd",
            d: "M20 5.50012V17.0396C20 17.8632 19.7562 18.6685 19.2993 19.3538C18.8966 19.9579 18.3432 20.4464 17.6938 20.7711L17.1177 21.0592C16.5932 21.3214 16.0361 21.5126 15.4611 21.6276L14.907 21.7384C14.3855 21.8427 13.857 21.9084 13.3259 21.9349L12 22.0012L10.6741 21.9349C10.143 21.9084 9.61449 21.8427 9.09298 21.7384L8.53892 21.6276C7.96392 21.5126 7.40681 21.3214 6.88233 21.0592L6.30623 20.7711C5.65684 20.4464 5.10342 19.9579 4.70069 19.3538C4.2438 18.6685 4 17.8632 4 17.0396V5.50012C4 3.56713 7.58172 2 12 2C16.4183 2 20 3.56713 20 5.50012ZM18.0254 5.35937L18.0262 5.35759C18.0262 5.35759 18.025 5.35944 18.0233 5.36313L18.0254 5.35937ZM16.8552 4.85744C17.3982 5.09501 17.7303 5.32916 17.9057 5.5C17.7303 5.67085 17.3982 5.90499 16.8552 6.14256C15.715 6.64139 14 7 12 7C9.99999 7 8.28496 6.64139 7.14478 6.14256C6.60177 5.90499 6.26966 5.67085 6.09426 5.5C6.26966 5.32915 6.60177 5.09501 7.14478 4.85744C8.28496 4.35861 9.99999 4 12 4C14 4 15.715 4.35861 16.8552 4.85744ZM9.7929 11.2929C10.1834 10.9024 10.8166 10.9024 11.2071 11.2929C11.5976 11.6834 11.5976 12.3166 11.2071 12.7071L9.41422 14.5L11.2071 16.2929C11.5976 16.6834 11.5976 17.3166 11.2071 17.7071C10.8166 18.0976 10.1834 18.0976 9.7929 17.7071L7.50503 15.4192C6.99735 14.9116 6.99735 14.0884 7.50503 13.5808L9.7929 11.2929ZM14.2071 11.2929C13.8166 10.9024 13.1834 10.9024 12.7929 11.2929C12.4024 11.6834 12.4024 12.3166 12.7929 12.7071L14.5858 14.5L12.7929 16.2929C12.4024 16.6834 12.4024 17.3166 12.7929 17.7071C13.1834 18.0976 13.8166 18.0976 14.2071 17.7071L16.495 15.4192C17.0027 14.9116 17.0027 14.0884 16.495 13.5808L14.2071 11.2929Z"
        }));
    }));
    var c = [ "default", "outlined", "filled" ];
    var d = function e(t) {
        return c.indexOf(t) >= 0;
    };
    var f = function e(t) {
        return t === "default" || t && !d(t) ? "outlined" : t;
    };
    /**
 * @description Data Source (ACIES) represents a source of datasets
 * @category Data Type
 * @keywords database, dataset, storage
 * @variants outlined,filled
 */    var v = function e(t) {
        var C = t.children, u = t.variant, c = u === void 0 ? "default" : u, d = o(t, [ "children", "variant" ]);
        var v = f(c);
        var s = "cylinderchevrons-".concat(v);
        var L = (0, r.useContext)(a.IconContext);
        var p = i.get(v);
        if (false) {}
        if (L && p) {
            var m = L.toRender, y = L.addIcon;
            y(s, p());
            if (!m) {
                return null;
            }
        }
        
        return n().createElement(l(), d, C, L ?  n().createElement("use", {
            href: "#".concat(s)
        }) : !!p && p());
    };
    /* harmony default export */    const s = v;
    module.exports = t;
    /******/})();